...
#   endif /* HAVE_ERROR_CHECKING */
    ...
    //What threshold to use for friendships among the processes
       char *nbr_frndshp_thr_env = NULL;
       nbr_frndshp_thr_env = getenv("SHM_NBR_FRNDSHP_THR");
       if(nbr_frndshp_thr_env == NULL)
           nbr_frndshp_thr = 8; //set to 1 by default
       else
           nbr_frndshp_thr = atoi(nbr_frndshp_thr_env);
       nbr_frndshp_thr++;

       char *k_env = NULL;
       k_env = getenv("SMGM_NBR_K_THR");
       if(k_env == NULL)
    	   K_vrbl = 4; //set to 1 by default
       else
    	   K_vrbl = atoi(k_env);
       //K_vrbl=K_vrbl+1;

       char *NBR_IMPL = NULL;
       NBR_IMPL = getenv("SMGM_NBR_IMPL");
       if(NBR_IMPL == NULL)
    	   nbr_impl = 0; //set to 0 by default
       else
    	   nbr_impl = atoi(NBR_IMPL);

       char *Topo_Aware = NULL;
       Topo_Aware = getenv("TOPO_AWARE_IMPL");
       if(Topo_Aware == NULL)
    	   topo_aware = 0; //set to 0 by default
       else
    	   topo_aware = atoi(Topo_Aware);
