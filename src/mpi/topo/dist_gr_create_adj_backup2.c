...
{
    char outfile[256];
    sprintf(outfile, "results/%d", rank);
...
//	printf("rank%d: 3.3\n", self_rank);
	...
	for(i=0;i<K_vrbl;i++)
	{
		...
		{
			//printf("rank%d: 3.3.1. i=%d, max_cmn_nbr_index=%d\n", self_rank, i, max_cmn_nbr_index);
		...
		else
		{
			if(order_flag==0)
			{
				//printf("rank%d: 3.3.2. i=%d\n", self_rank, i);
				sndreq.grp[i]=self_rank;
				order_flag=1;
			}
			else
			{
				//printf("rank%d: 3.3.3. i=%d, max_cmn_nbr_index=%d\n", self_rank, i, max_cmn_nbr_index);
		...
		while(1)
	{
		...
		//printf("rank%d: receive %d from process %d\n", self_rank, recvreq.msgtype, status.MPI_SOURCE);
				...
				max_cmn_nbr_index=-1, max_val=0;
					...
					for(j=0;j<K_vrbl;j++)
					{
						if(sndreq.grp[j]!=self_rank)
						{
							...
							//printf("*rank%d: send %d to process %d\n", self_rank, sndreq.msgtype, sndreq.grp[j]);
				...
				if(avail_grpfrnds->num_cmn_brs[i]!=-1)
				{
					for(p=0;p<K_vrbl;p++)
					{
						if(flag_ranks[avail_grpfrnds->grpfriends[p][i]]==0)
						{
							...
							//printf("rank%d: send %d to process %d\n", self_rank, sndreq.msgtype, avail_grpfrnds->grpfriends[p][i]);
						}
...
int MPIR_Make_GroupFriend_Matrix(MPID_Comm *comm_ptr, Individual_Friendship_Matrix *ind_frnd_mat, Group_Friendship_Matrix **grp_frnd_mat_ptr)
{
	...
	int mpi_errno = MPI_SUCCESS;
	int i, j,p,m,n, comm_size, self_rank, indegree,outdegree;
	comm_size = comm_ptr->local_size;
	self_rank = comm_ptr->rank;
	int cnt_cmn_nbrs;
	MPIU_CHKPMEM_DECL(9);
...
/*	int *k_index, pk=0, num_and=K_vrbl-1;
	k_index = (int*) malloc (num_and+1);

	for(int a=0;a<num_and+1;a++)
		k_index[a]=0;*/

	//if(self_rank==0)
	//printf("numfriends=%d\n", ind_frnd_mat->numfriends);

	int counter=0;

	//printf("rank%d: num friends=%d\n", self_rank, ind_frnd_mat->numfriends);

/*	if(ind_frnd_mat->numfriends>K_vrbl-2)
	{
		//printf("rank%d: in loop\n", self_rank);
	while (k_index[num_and]==0) {
		...
		if(cnt==num_and)
		{
			...
			for(p=0;p<outdegree;p++)
			{
				...
				for(j=0;j<num_and;j++)
				{
				result_and=result_and &ind_frnd_mat->frndship_bit_map[k_index[j]][p];
				//if(self_rank==0)
					//printf("result_and=%d, ind_frnd_mat->frndship_bit_map[%d][%d]=%d\n", result_and, k_index[j],p,ind_frnd_mat->frndship_bit_map[k_index[j]][p]);
				}
				...
				if(result_and>0)
				{
					cnt_cmn_nbrs++;
				//	if(self_rank==0)
					//	printf("result_and for i=%d, j=%d, m=%d,  p=%d is %d= %d & %d\n",  k_index[2], k_index[1], k_index[0], p, result_and, ind_frnd_mat->frndship_bit_map[k_index[0]][p], ind_frnd_mat->frndship_bit_map[k_index[1]][p], ind_frnd_mat->frndship_bit_map[k_index[2]][p] );
	...
	//	if(ind_frnd_mat->numfriends==0)
	//		goto exit_loop1;

	//	int count2=0;
		 while(k_index[pk]==ind_frnd_mat->numfriends) {
			 k_index[pk]=0;
			 k_index[++pk]++;

		//	 printf("**rank%d: k_index[%d]=%d\n", self_rank, pk, k_index[pk]);

			 if(k_index[pk]!=ind_frnd_mat->numfriends) {
				 pk=0;
			 }

			// printf("rank%d: count%d, k_index[%d]=%d\n", self_rank, count2, pk,k_index[pk]);
		//	 count2++;
		 }
	...
	//	 printf("rank%d: k_index[%d]=%d\n", self_rank, num_and, k_index[num_and]);
		/* if(counter>20)
		 {
			 printf("rank%d stuck in loop\n", self_rank);
			 goto exit_loop2;
		 }*/
/*	}
	//printf("rank%d: out loop\n", self_rank);
	}
	//exit_loop2:
	//printf("rank%d: out of loop\n", self_rank);
	// MPI_Barrier(MPI_COMM_WORLD);
	//if(self_rank==0)
		//printf("rank%d: num_grp_frnds=%d\n", grp_frnd_mat->num_grp_frnds);
	//printf("rank%d\n", self_rank);

	if(grp_frnd_mat->num_grp_frnds>10000000)
		printf("Not Enough Memory. Reduce the Threshold\n");
	...
	while (k_index[num_and]==0) {
			...
			if(cnt==num_and)
			{
				...
				for(p=0;p<outdegree;p++)
				{
					...
					if(result_and>0)
					{
						cnt_cmn_nbrs++;
					//	if(self_rank==0)
						//	printf("result_and for i=%d, j=%d, p=%d is %d= %d & %d\n",  k_index[1], k_index[0], p, result_and, ind_frnd_mat->frndship_bit_map[k_index[0]][p], ind_frnd_mat->frndship_bit_map[k_index[1]][p] );
		...
		/*			}
				...
				if(cnt_cmn_nbrs>=nbr_frndshp_thr)
				{
					...
					for(p=0;p<outdegree;p++)
					{
						grp_frnd_mat->grpfrnd_bit_map[p][q]=1;
					for(j=0;j<num_and;j++)
					{
						grp_frnd_mat->grpfrnd_bit_map[p][q]=grp_frnd_mat->grpfrnd_bit_map[p][q]&ind_frnd_mat->frndship_bit_map[k_index[j]][p];
					}

				//	if(self_rank==0)
					//	printf("grpfrnd_bit_map[%d][%d]=%d\n", p, q, grp_frnd_mat->grpfrnd_bit_map[p][q]);
				...
				/*	if(self_rank==0)
					{
						for(p=0;p<outdegree;p++)
						{
							printf("num_actv_grp_of_nbr[%d]=%d\n", p, grp_frnd_mat->num_actv_grp_of_nbr[p]);
						}
					}*/

			/*		for(j=0;j<num_and;j++)
					{
						grp_frnd_mat->grpfriends[num_and-j-1][q]=ind_frnd_mat->friends[k_index[j]];
				...
				/*	if(self_rank==0)
					{
						for(j=0;j<num_and;j++)
						printf("grpfriends[%d][%d]=%d\n", j, q, grp_frnd_mat->grpfriends[j][q]);
					}*/
		/*			q++;
				}
...
/*	int *num_proc_per_matrix;
	MPIU_CHKPMEM_MALLOC(num_proc_per_matrix, int*, comm_size * sizeof(int*), mpi_errno, "tmp_bit_map");

	int matrix_size;
	if(ind_frnd_mat->numfriends>MAX_COMB_DEGREE*comm_size)
		matrix_size=ind_frnd_mat->numfriends;
	else
		matrix_size=MAX_COMB_DEGREE*comm_size;

	int **tmp_bit_map;
	MPIU_CHKPMEM_MALLOC(tmp_bit_map, int**, matrix_size * sizeof(int*), mpi_errno, "tmp_bit_map");

	for(i=0;i<num_frnds;i++)
		tmp_bit_map[i] = MPIU_Malloc(outdegree * sizeof(int));

	for(i=0;i<ind_frnd_mat->numfriends;i++)
		for(j=0;j<outdegree;j++)
			tmp_bit_map[i][j]=ind_frnd_mat->frndship_bit_map[i][j];

	double tmp_num_matrix=(double)(log((int) (K_vrbl-1))/(double) log(2));
	int num_matrix=tmp_num_matrix+ 1e-9;


	int tot_matrix_size=0,k, upper_bound;

	for(k=0;k<num_matrix;k++)
	{
		for(i=0;i<comm_size;i++)
			num_proc_per_matrix[i]=0;

		if(k==0)
			upper_bound=ind_frnd_mat->numfriends;
		else
			upper_bound=tot_matrix_size;
	...
	for(i=0;i<upper_bound;i++)
		{
			for(j=i+1;j<upper_bound;j++)
			{
				...
				for(p=0;p<outdegree;p++)
				{
					if(tmp_bit_map[i][p]&tmp_bit_map[j][p])
					{
						cnt_cmn_nbrs++;
					}
				}
				...
				if(cnt_cmn_nbrs>=nbr_frndshp_thr)
				{
					if(num_proc_per_matrix[ind_frnd_mat->friends[i]]<MAX_COMB_DEGREE &&  num_proc_per_matrix[ind_frnd_mat->friends[j]]<MAX_COMB_DEGREE)
					{
						tot_matrix_size++;
						num_proc_per_matrix[ind_frnd_mat->friends[i]]++;
						num_proc_per_matrix[ind_frnd_mat->friends[j]]++;
					}
				}
			}
		}
			...
			for(p=0;p<outdegree;p++)
			{
				if(ind_frnd_mat->frndship_bit_map[i][p]&ind_frnd_mat->frndship_bit_map[j][p])
				{
					...
					/*if(self_rank==0)
					{
						...
						printf("***result_and for i=%d, j=%d, p=%d is %d= %d & %d\n", i,j, p, ind_frnd_mat->frndship_bit_map[i][p]&ind_frnd_mat->frndship_bit_map[j][p], ind_frnd_mat->frndship_bit_map[i][p], ind_frnd_mat->frndship_bit_map[j][p]);
						//printf("***result_and for %d , %d is %d= %d & %d\n", j, p, ind_frnd_mat->frndship_bit_map[j][p]&ind_frnd_mat->frndship_bit_map[j][p], ind_frnd_mat->frndship_bit_map[i][p], ind_frnd_mat->frndship_bit_map[j][p]);
						//printf("2.i=%d, j=%d\n", j, p);
						//printf("2. done\n");
					}*/
				}
	...
	for(i=0;i<ind_frnd_mat->numfriends;i++)
		{
			for(j=i+1;j<ind_frnd_mat->numfriends;j++)
			{
				...
				/*	if(self_rank==0)
					...
					/*	if(self_rank==0)
						{
							printf("***grpfriends[0][%d]=%d\n", q, grp_frnd_mat->grpfriends[0][q]);
							printf("***grpfriends[1][%d]=%d\n", q, grp_frnd_mat->grpfriends[1][q]);
						}*/
...
//	grp_frnd_mat->num_grp_frnds=0;
	if(K_vrbl==4)
	{
		for(i=0;i<ind_frnd_mat->numfriends;i++)
			{
				for(j=i+1;j<ind_frnd_mat->numfriends;j++)
				{
					...
					for(p=0;p<outdegree;p++)
					{
						...
						//	if(self_rank==0)
							//	printf("result_and for i=%d, j=%d, m=%d, p=%d is 1= %d & %d & %d\n",  i, j, m, p, ind_frnd_mat->frndship_bit_map[i][p], ind_frnd_mat->frndship_bit_map[j][p], ind_frnd_mat->frndship_bit_map[m][p] );
		...
		//if(self_rank==0)
		//printf("2.rank%d: num_grp_frnds=%d\n", grp_frnd_mat->num_grp_frnds);
		...
		for(i=0;i<ind_frnd_mat->numfriends;i++)
		{
			for(j=i+1;j<ind_frnd_mat->numfriends;j++)
			{
				for(m=j+1;m<ind_frnd_mat->numfriends;m++)
				{
					...
					for(p=0;p<outdegree;p++)
					{
						if(ind_frnd_mat->frndship_bit_map[i][p]&ind_frnd_mat->frndship_bit_map[j][p]&ind_frnd_mat->frndship_bit_map[m][p]&ind_frnd_mat->frndship_bit_map[n][p])
							cnt_cmn_nbrs++;
					}
			...
			for(j=i+1;j<ind_frnd_mat->numfriends;j++)
			{
				for(m=j+1;m<ind_frnd_mat->numfriends;m++)
				{
					...
					if(cnt_cmn_nbrs>=nbr_frndshp_thr)
					{
						for(p=0;p<outdegree;p++)
						{
							grp_frnd_mat->grpfrnd_bit_map[p][q]=ind_frnd_mat->frndship_bit_map[i][p]&ind_frnd_mat->frndship_bit_map[j][p]&ind_frnd_mat->frndship_bit_map[m][p]&ind_frnd_mat->frndship_bit_map[n][p];
							if(grp_frnd_mat->grpfrnd_bit_map[p][q])
								grp_frnd_mat->num_actv_grp_of_nbr[p]++;
						}
						//grp_frnd_mat->grpfriends[0][q]=self_rank;
...
int MPIR_Make_Friendship_Matrix(MPID_Comm *comm_ptr, Common_nbrhood_matrix *cmn_nbh_mat, Individual_Friendship_Matrix **ind_frnd_mat_ptr)
{
	...
	for(i = 0; i < cmn_nbh_mat->num_rows; i++)
	{
		...
		{
			if(cmn_nbh_mat->matrix[i][j] != self_rank)
			{
...
int MPIR_Get_inNbrs_of_outNbrs(MPID_Comm *comm_ptr, Common_nbrhood_matrix **cmn_nbh_mat_ptr)
{
 ...
 /*   if(comm_ptr->rank==0)
    {
    for(my_i=0;my_i<outdegree;my_i++)
    	printf("rank%d: Outgoing neighbor %d=%d\n", comm_ptr->rank, my_i, topo_ptr->topo.dist_graph.out[my_i]);

    for(my_i=0;my_i<indegree;my_i++)
    	printf("rank%d: incoming neighbor %d=%d\n", comm_ptr->rank, my_i, topo_ptr->topo.dist_graph.in[my_i]);
...
int MPIR_Build_SHM_nbh_coll_patt(MPID_Comm *comm_ptr)
{
   ...
   // printf("rank%d - PID%d: 0.\n", self_rank, getpid());
    ...
    //Scheduler_Info *scheduler_matrix;
    //MPIU_CHKPMEM_MALLOC(scheduler_matrix, Scheduler_Info*, sizeof(Scheduler_Info), mpi_errno, "scheduler_matrix");
  /*  MPIU_CHKPMEM_CALLOC(scheduler_matrix->comb_matrix, Comb_element**, outdegree* sizeof(Comb_element*), mpi_errno, "scheduler_matrix->comb_matrix");
    MPIU_CHKPMEM_CALLOC(scheduler_matrix->num_entries_in_row, int*, outdegree* sizeof(int), mpi_errno, "scheduler_matrix->num_entries_in_row");

    for(i=0;i<outdegree;i++)
    {
    	scheduler_matrix->comb_matrix[i] = MPIU_Malloc(MAX_COMB_DEGREE * sizeof(Comb_element));
     }

    for(i=0;i<outdegree;i++)
    	for(j = 0; j < MAX_COMB_DEGREE; j++)
    {
    MPIU_CHKPMEM_CALLOC(scheduler_matrix->comb_matrix[i][j].grp_frnds, int*, K_vrbl* sizeof(int), mpi_errno, "scheduler_matrix->comb_matrix[i][j].grp_frnds");
    MPIU_CHKPMEM_CALLOC(scheduler_matrix->comb_matrix[i][j].opt, Operation*, K_vrbl* sizeof(Operation), mpi_errno, "scheduler_matrix->comb_matrix[i][j].opt");
    }
    ...
    for(i=0;i<outdegree;i++)
    {
    	for(j = 0; j < MAX_COMB_DEGREE; j++)
    	{
    		for(k=0;k<K_vrbl;k++)
    		{
    			scheduler_matrix->comb_matrix[i][j].opt[k] = IDLE;
    			scheduler_matrix->comb_matrix[i][j].grp_frnds[k] = -1;
    		}
    	}
    	scheduler_matrix->num_entries_in_row[i]=0;
    }*/
  ...
  //  printf("rank%d: 2.\n", self_rank);
    ...
    Group_Friendship_Matrix *grp_frnd_mat;
    MPIR_Make_GroupFriend_Matrix(comm_ptr,ind_frnd_mat, &grp_frnd_mat);
   ...
   /* for(j=0;j<grp_frnd_mat->Total_num_grp_frnds;j++)
    {
    	printf("rank%d: group%d= %d %d\n ", self_rank, j, grp_frnd_mat->grpfriends[0][j], grp_frnd_mat->grpfriends[1][j]);
    }*/
  ...
  /*  if(slcted_grp_frnd.is_grp==1 && grp_frnd_mat->num_grp_frnds!=0)
    {
    	//if(self_rank==0)
    	printf("***rank%d- step%d: (%d, %d)\n", self_rank, steps, slcted_grp_frnd.grp_frnds[0], slcted_grp_frnd.grp_frnds[1]);
    }*/
 /*   else
    {
    	if(self_rank==0)
    	printf("***rank%d- step%d: no friend\n", self_rank, steps);
    }*/
    ...
    if(slcted_grp_frnd.is_grp==1 && grp_frnd_mat->num_grp_frnds!=0)
    {
    	...
    	//int i1;
    	//for(i1=0;i1<grp_frnd_mat->num_cmn_brs[slct_grp_indx];i1++)
    		//printf("")
    	...
    	//printf("rank%d: index of selected grp friend=%d\n", self_rank, slct_grp_indx);
    	...
    	for(i=0;i<K_vrbl;i++)
    	{
    		if(my_indx==i) //keep the common neighbors
    		{
    			...
    			//cmn_nbh_mat->is_row_offloaded[cmn_nbrs[j].index]=2;
    			...
    			for(j=start_on_indx;j<end_on_indx;j++)
    			{
    				...
    				for(k=0;k<K_vrbl;k++)
    				{
    					...
    					if(slcted_grp_frnd.grp_frnds[k]!=self_rank)
    					{
    						...
    						// printf("rank%d: comb_matrix[%d][%d].grp_frnds[%d]=%d\n", self_rank,cmn_nbrs[j].index, cmn_nbh_mat->t, k_grpfrnd, cmn_nbh_mat->comb_matrix[cmn_nbrs[j].index][cmn_nbh_mat->t].grp_frnds[k_grpfrnd]);
    						// if(self_rank==0)
    						// printf("(recv) cmn_nbh_mat->comb_matrix[%d][%d].grp_frnds[%d]=%d\n", cmn_nbrs[j].index, cmn_nbh_mat->t, k_grpfrnd, cmn_nbh_mat->comb_matrix[cmn_nbrs[j].index][cmn_nbh_mat->t].grp_frnds[k_grpfrnd] );
    						 k_grpfrnd++;
    					}
    				}
    				//printf("rank%d: I'm responsible for %d, %d, %d for common neighbor %d\n", self_rank, slcted_grp_frnd.grp_frnds[0], slcted_grp_frnd.grp_frnds[1], slcted_grp_frnd.grp_frnds[2], cmn_nbrs[j].rank);
    						//add_frnd_to_comb_matrix(scheduler_matrix, cmn_nbrs[j].index, slcted_grp_frnd.grp_frnds[k] , RECV, steps, k);
    			}
    		}
    		else	//offload common neighbors to other friends
    		{
    			...
    			for(j=start_off_indx;j<end_off_indx;j++)
    			{
    				...
    				//if(self_rank==0)
    				//printf("(send) cmn_nbh_mat->comb_matrix[%d][%d].grp_frnds[%d]=%d\n", cmn_nbrs[j].index, cmn_nbh_mat->t, i_grpfrnd, cmn_nbh_mat->comb_matrix[cmn_nbrs[j].index][cmn_nbh_mat->t].grp_frnds[i_grpfrnd] );
    			}
    			i_grpfrnd++;
    		}
    	}
   ...
   //   if(self_rank==0)
     ...
     // printf("rank%d reached the end\n", self_rank);
      ...
      for(i = 0; i < outdegree; i++)
      {
    	...
    	/*  if(self_rank==0)
    	  {
    		  for(j=0;j<K_vrbl+1;j++)
    		  {
    		  printf("rank%d: sched_msg[%d][%d]=%d  \n", self_rank, i, j, sched_msg[i][j]);
    		  }
    		 // printf("\n");
    	  }*/
      }
    ...
    /*	  if(dests[i]==0)
    	  {
    		  for(ii=0;ii<K_vrbl+1;ii++)
    			  printf("rank %d sent sched_msg[%d][%d]=%d to rank 0\n", self_rank,i, ii, sched_msg[i][ii]);
  ...
  /*    if(self_rank==0)
      {
    	  for(i=0;i<indegree;i++)
    	  {
    		  for(ii=0;ii<K_vrbl+3;ii++)
    		  {
    			  printf("sched_recv_buffs[%d][%d]=%d\n", i, ii, sched_recv_buffs[i][ii]);
    		  }
    	  }
      }*/
