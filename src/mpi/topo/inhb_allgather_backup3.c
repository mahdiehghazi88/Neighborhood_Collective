	 ...
	 // printf("rank%d: cmn_nbh_mat->t=%d\n", self_rank, cmn_nbh_mat->t);
	    			 ...
	    			 // if(cmn_nbh_mat->comb_matrix[i][t].opt[k] == RECV && flag_recv==0)
	    			  {
	    				...
	    				/*  if(self_rank==0)
	    				  {
	    					  printf("0.rank%d pid%d: t=%d, receivebuff from source %d is %d\n", self_rank, getpid(), t, cmn_nbh_mat->comb_matrix[i][t].grp_frnds[k],  recvcount);
	    				  }*/
	    				//  if(self_rank==0 )
	    					//  cnt_recv_ind++;
	    			...
	    			//  if(cmn_nbh_mat->comb_matrix[i][t].opt[k] == SEND && flag_send==0)
	    			  {
	    				...
	    				//  if(cmn_nbh_mat->comb_matrix[i][t].grp_frnds[k]==0)
	    					//  printf("0.rank%d pid%d: t=%d, sendbuff to dest %d is %d\n", self_rank, getpid(), t, cmn_nbh_mat->comb_matrix[i][t].grp_frnds[k],  sendcount);
	    		  ...
	    		  if(!incom_sched_mat[i][0]) //On incoming neighbors
	    		  {
	    			//  if(self_rank==0)
	    			  ...
	    			  if(incom_sched_mat[i][1] == t)
	    			  {
	    				...
	    				//  if(self_rank==4)
	    				//  printf("2.rank%d pid%d: t=%d, receivebuff from source %d is %d\n", self_rank, getpid(), t, srcs[i],  incom_recv_count);
	  ...
	  //  printf("rank%d: 2.\n", getpid());
	    /* Now we might have some ON neighbors that we
	        ...
	        for(i = 0; i < cmn_nbh_mat->num_rows; i++)
	        {
	            ...
	            if(!cmn_nbh_mat->ignore_row[i]) //out neighbor still active
	            {
	               ...
	               // if(dests[i]==4)
	              //  if(dests[i]==0)
	              //  printf("3.rank%d pid%d: sends directly to dest %d \n", self_rank, getpid(),dests[i]);
	                //if(dests[i]==0)
	    ...
	    for(i = 0; i < indegree; i++)
	    {
	    	...
	    	//	if(self_rank==8)
	    		//printf("4.rank%d pid%d: receivebuff from source %d is %d\n", self_rank, getpid(),srcs[i],  rb);
	    		//if(self_rank==0)
