	 ...
	 // printf("rank%d: cmn_nbh_mat->t=%d\n", self_rank, cmn_nbh_mat->t);
	    	  ...
	    	  for(i = 0; i < cmn_nbh_mat->num_rows; i++)
	    	  {
	    		int  flag=0;
	    		  ...
	    		  if(cmn_nbh_mat->comb_matrix[i][t].opt[k] != IDLE)
	    		  {
	    			...
	    			//  for(k=0;k<K_vrbl-1;k++)
	    			  //{
	    			  if(cmn_nbh_mat->comb_matrix[i][t].opt[k] == RECV)
	    			  {
	    				...
	    				//  if(self_rank==0)
	    					 // printf("0.rank%d pid%d: receivebuff from source %d is %d. buffer size is %d\n", self_rank, getpid(), cmn_nbh_mat->comb_matrix[i][t].grp_frnds[k],  recvcount, sizeof(rb));
	    			  ...
	    			  if(cmn_nbh_mat->comb_matrix[i][t].opt[k] == SEND )
	    			  {
	    				 ...
	    				 // if(cmn_nbh_mat->comb_matrix[i][t].grp_frnds[k]==0)
	    					//  printf("0.rank%d pid%d: sendbuff to dest %d is %d. buffer size is %d\n", self_rank, getpid(),cmn_nbh_mat->comb_matrix[i][t].grp_frnds[k],  sendcount, sizeof(sendbuf));
	    						...
	    						//  printf("rank%d: j=%d, t=%d\n", self_rank, j, t);
	    						 // if(!cmn_nbh_mat->is_row_offloaded[j] && cmn_nbh_mat->comb_matrix[j][t].opt[0] == RECV)
	    						  //{
	    							...
	    							//  if(dests[i]==0)
	    						//	  printf("1.rank%d pid%d: sendbuff to dest %d is %d. buffer size is %d\n", self_rank, getpid(),dests[i],  sendcount*K_vrbl, sizeof(exchange_tmp_buf));
	    							  //  if(dests[j]==0)
	    							  {
	    								 // printf("rank%d pid%d sent the combined message to rank %d, j=%d, t=%d, k=%d \n", self_rank,getpid(), dests[i], i, t, k);
	    								  sender_rank=self_rank;
	    							  }
	    			...
	    			//  if(self_rank==0)
	    			  ...
	    			  if(incom_sched_mat[i][1] == t)
	    			  {
	    				...
	    				//  if(self_rank==0)
	    				 ...
	    				 // if(self_rank==0)
	    				//  printf("2.rank%d pid%d: receivebuff from source %d is %d. buffer ise is %d\n", self_rank, getpid(),srcs[i],  incom_recv_count, sizeof(rb));
	  ...
	  //  printf("rank%d: 2.\n", getpid());
	    /* Now we might have some ON neighbors that we
	        ...
	        for(i = 0; i < cmn_nbh_mat->num_rows; i++)
	        {
	            ...
	            if(!cmn_nbh_mat->ignore_row[i]) //out neighbor still active
	            {
	                ...
	                if (mpi_errno) MPIU_ERR_POP(mpi_errno);
	              //  if(dests[i]==0)
	              //  printf("3.rank%d pid%d: sendbuff to dest %d is %d. buffer size is %d\n", self_rank, getpid(),dests[i],  sendcount, sizeof(sendbuf));
	                //if(dests[i]==0)
	    ...
	    for(i = 0; i < indegree; i++)
	    {
	    	...
	    	//	if(self_rank==0)
	    		//printf("4.rank%d pid%d: receivebuff from source %d is %d. buffer size is %d\n", self_rank, getpid(),srcs[i],  incom_recv_count, sizeof(rb));
	    		//if(self_rank==0)
